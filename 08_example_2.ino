...
// turn on the LED if the distance is between dist_min and dist_max
  if(dist_raw < 100 or dist_raw > 300) {
    analogWrite(PIN_LED, 255);
  }
  else if(dist_min <= dist_raw and dist_raw < 200.0){
    a = (200.0 - dist_raw) * (2.55);
    analogWrite(PIN_LED, a);
  }
  else if (200.0 <= dist_raw and dist_raw <= dist_max){
    a = (dist_raw - 200.0) * (2.55);
    analogWrite(PIN_LED, a);
  }
...
{
  float reading, l_reading;
  digitalWrite(TRIG, HIGH);
  delayMicroseconds(10);
  digitalWrite(TRIG, LOW);
  reading = pulseIn(ECHO, HIGH, timeout) * scale; // unit: mm
  if (not(reading < dist_min or reading > dist_max))
  l_reading = reading;
  if(reading < dist_min or reading > dist_max) {
       reading = l_reading;
  }
